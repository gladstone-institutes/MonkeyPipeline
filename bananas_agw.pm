...
sub buildJobSubmissionList {
	...
	if (not $isEdgeRBeingDone) {
		...
		setJobInfo($cfg, $STEP_SUMMARY_AFTER_EDGER, "all", $dep,$vars,$cfg->{r}->{monkeyPoo}, "51b.summary.R");
		setJobInfo($cfg, $STEP_CLUSTER_AFTER_EDGER, "all", $dep,$vars,$cfg->{r}->{monkeyPoo}, "51c.cluster.R"); # output goes into the summaryFigureDir
