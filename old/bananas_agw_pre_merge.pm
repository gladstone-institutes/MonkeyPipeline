...
sub checkConfig {
    ...
    $cfg->{bin}->{peakMotifs} = "/data/applications/rsat/rsat/perl-scripts/peak-motifs"; # hard-coded ???
...
sub setJobInfo {
    ...
    ($jobName !~ /\./) or confess "The job name CANNOT CONTAIN A PERIOD / decimal point, but we had a job named '$jobName' that contained such an illegal character! Quitting. ";
	...
	($jobName ne $dep) or confess "[ERROR in setting up qsub jobs]: You set the job name ($jobName) to THE SAME NAME as one of its prerequisites! This is not valid--a job cannot depend on itself. Fix it! ";
    }
	...
	($value !~ "[,]") or confess "[ERROR in setting up qsub jobs: COMMA NOT ALLOWED]: Note: Torque/qsub is VERY PARTICULAR and cannot accept a comma in an argument, even if it's inside double quotes. You have to REMOVE the comma, use some kind of placeholder text, and then manually re-add it in the script that you called. There is no other workaround. I recommend using <COMMA> and then replacing that with a ',' in the being-called script.";
	$variables .= "," . "$key='$value'"; # <-- Variables are comma-delimited and are surrounded by single quotation marks.
    }
    ...
    # ============ DONE! ==================
    my $qsub = "${jobName}=" . '`' . "qsub -N $jobName " . $dependencies . " " . $variables . " " . $scriptFullPath . '`'; # <-- note the literal backtick! There will be a matching one later. Also: the saving-to-shell-variables part is SPACE-SENSITIVE, so DO NOT add spaces here whatever you do.
    $globalNumJobsSubmitted++;
    debugPrint("[DEBUG] Added job ${globalNumJobsSubmitted} via qsub: $qsub\n", "cyan");
    return($jobName,$qsub);
}
...
sub buildJobSubmissionList {
	...
	foreach my $k (keys %$h) { # 'k' is the sample name--for example "DRUG.3" or "Wildtype.1"
	    ...
	    my $vars = { 'filterDir'  => $cfg->{filterResultsDir}  ,
			 ...
			 'force'      => $FORCE_RERUN ,
			 'verbose'    => $GLOBAL_VERBOSE , 'debug'=>$GLOBAL_DEBUG ,
			 'sampleName' => $k ,
			 'inputFile1' => $sKeys[0] , 
	    ...
	    if (scalar(@sKeys) == 2) { $vars->{inputFile2} = $sKeys[1]; } # <-- for paired end, apparently get the second file this way
	...
	foreach my $h ($sampleHash,$inputHash) {
	    next if (!defined($h)); # Skip any undefined 'inputHash' cases.
	    foreach my $k (keys %$h) {
		my $jobName = sanitizeJobName($cfg->{studyName} . "_s05.1_" . $k);
		...
		$cfg->{jobs}->{'05.1_tags'}->{$k}->{jobName} = $jn;
		$cfg->{jobs}->{'05.1_tags'}->{$k}->{qsub} = $qs;
	    }
	}
    }
    ...
    # Tag mapping stats
    if (not $cfg->{skipTagsAndDensity}) {
	my $jobName = sanitizeJobName($cfg->{studyName} . "_s05.2_xSummary"); # job names must be strictly in dependency alphabetical order!
	my $dep;
	my $th = $cfg->{jobs}->{'05.1_tags'};
	foreach my $k (sort keys %$th) {
	    $dep->{$k} = $cfg->{jobs}->{'05.1_tags'}->{$k}->{jobName};
	}
	my $vars = { 'force' => $FORCE_RERUN, 'verbose' => $GLOBAL_VERBOSE, 'debug'=>$GLOBAL_DEBUG };
	$vars->{genome} = $cfg->{genomeName};
	...
	$cfg->{jobs}->{'05.2_xSummary'}->{'xSummary'}->{jobName} = $jn;
	$cfg->{jobs}->{'05.2_xSummary'}->{'xSummary'}->{qsub}    = $qs;
    }
	...
	foreach my $k (keys %$sampleHash) {
	    ...
	    my $dep = { 'tagsJob' => $cfg->{jobs}->{'05.1_tags'}->{$k}->{jobName} };
	    my $vars = { 'force' => $FORCE_RERUN, 'verbose' => $GLOBAL_VERBOSE, 'debug'=>$GLOBAL_DEBUG };
	    $vars->{genome} = $cfg->{genomeName};
	    $vars->{seqType} = $cfg->{details}->{$k}->{type};
		...
		$dep->{inputJob} = $cfg->{jobs}->{'05.1_tags'}->{$vars->{inputName}}->{jobName};
	    }
	...
	foreach my $k (keys %$sampleHash) {
		...
		my $jobName = sanitizeJobName($cfg->{studyName} . "_s07.0_" . $k);
		my $dep;
		...
		$cfg->{jobs}->{'07.1_peaks'}->{$k}->{jobName} = $jn;
		$cfg->{jobs}->{'07.1_peaks'}->{$k}->{qsub}    = $qs;
	    }
	}
	...
	my $jobName = $cfg->{studyName} . "_s07.1_xSummary";
	my $dep;
	my $th = $cfg->{jobs}->{'07.1_peaks'};
	foreach my $k (sort keys %$th) {
	    $dep->{$k} = $cfg->{jobs}->{'07.1_peaks'}->{$k}->{jobName};
	}
	my $vars = { 'force' => $FORCE_RERUN, 'verbose' => $GLOBAL_VERBOSE, 'debug'=>$GLOBAL_DEBUG };
	$vars->{genome} = $cfg->{genomeName};
	$vars->{bedmap} = $cfg->{bin}->{bedmap};
	...
	$cfg->{jobs}->{'07.2_xSummary'}->{'xSummary'}->{jobName} = $jn;
	$cfg->{jobs}->{'07.2_xSummary'}->{'xSummary'}->{qsub}    = $qs;
    }

    # Motif discovery
    if ($cfg->{doPeaks}) {
	foreach my $k (keys %$sampleHash) {
	    # Alex's note: note that we do NOT do motif discovery for type 'rna'! Just 'chip' and 'exo'.
            if ($cfg->{details}->{$k}->{type} eq "chip" || $cfg->{details}->{$k}->{type} eq "exo") {	
		my $jobName = sanitizeJobName($cfg->{studyName} . "_s10_" . $k);
		my $dep  = { "peaksJob"=>$cfg->{jobs}->{'07.1_peaks'}->{$k}->{jobName} }; # Depends on 07 peaks
		my $vars = { 'force' => $FORCE_RERUN, 'verbose' => $GLOBAL_VERBOSE, 'debug'=>$GLOBAL_DEBUG };
		$vars->{genome} = $cfg->{genomeName};
		$vars->{bedops} = $cfg->{bin}->{bedops};
		$vars->{bedtools} = $cfg->{bin}->{bedtools};
		$vars->{peakMotifs} = $cfg->{bin}->{peakMotifs};
		$vars->{fasta}   = $cfg->{genomeFasta}; # fasta file
	...
	my $jobName = sanitizeJobName($cfg->{studyName} . "_s08.2_agwbrowser_" . "everything");
	my $vars = {   'force'=>$FORCE_RERUN, 'verbose'=>$GLOBAL_VERBOSE, 'debug'=>$GLOBAL_DEBUG,
	...
	$cfg->{jobs}->{'48_browser_tracks'}->{"everything"} = { "jobName"=>$jn, "qsub"=>$qs }; # '49' so it gets alphabetized after all alignment is done, guaranteed.
    }

    if ($cfg->{browserBins}) {      # Sean's binned UCSC files -- generated if "browserBins" is specified
	foreach my $k (keys %$sampleHash) {
            my $jobName = sanitizeJobName($cfg->{studyName} . "_s08.1_" . $k);
	    my $dep;
	    $dep->{densityJob} = $cfg->{jobs}->{'06_density'}->{$k}->{jobName};
	    if ($cfg->{details}->{$k}->{type} eq "chip" || $cfg->{details}->{$k}->{type} eq "exo") {
		$dep->{peaksJob} = $cfg->{jobs}->{'07.1_peaks'}->{$k}->{jobName};
	    }
	    my $vars = { 'force' => $FORCE_RERUN, 'verbose' => $GLOBAL_VERBOSE, 'debug'=>$GLOBAL_DEBUG };
	    $vars->{genome} = $cfg->{genomeName};
	    $vars->{seqType} = $cfg->{details}->{$k}->{type};
	...
	# After the browser tracks have all been added to the queue, generate the SUMMARY...
        my $jobName = sanitizeJobName($cfg->{studyName} . "_s08.2_xSummary");
        my $dep; # hash reference
        my $th = $cfg->{jobs}->{'08_browser'};
        foreach my $k (sort keys %$th) {
	    $dep->{$k} = $cfg->{jobs}->{'08_browser'}->{$k}->{jobName}; # Add dependencies
        }
	...
	$cfg->{jobs}->{'08_xSummary'}->{'xSummary'}->{qsub}    = $qs; # The actual qsub command
    }
    ...
    # build qc jobs (conditional). Nothing depends on FastQC output, by the way, so this can be added last.
    unless ($cfg->{skipQC}) {
	foreach my $h ($sampleHash,$inputHash) {
	    next if (!defined($h)); # Skip any undefined 'inputHash' cases.
